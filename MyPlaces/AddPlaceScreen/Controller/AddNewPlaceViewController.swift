...
class AddNewPlaceViewController: UITableViewController {
    ...
    @IBOutlet weak var ratingControl: RatingButtons!
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        // Объяевляем MapVC как делегату его свойтв
        mapVC.mapViewControllerDelegate = self
        
        // Показываем место
        if identifier == "showPlace" {
            // Используется такой способ передечи данных, что бы приложение не падало при открытии карт на экране создания нового места
    ...
    // Метод отвечает за присвоение значений к экземпляру структуры, созданной в самом начале класса
    func saveController() {
        ...
        if editPlace != nil {
            ...
            try! realm.write {
                ...
                editPlace.rating = Double(newPlace.rating)
            }
    ...
    private func setupEditScreen() {
        ...
        if editPlace != nil {
            ...
            ratingControl.rating = Int(editPlace.rating)
...
// Расширение для передачи адреса с MapVC
extension AddNewPlaceViewController: MapViewControllerDelegate {
    
    // В этом обязательном методе уже есть значение address, которое мы передали ему в MapVC в IBAction кнопки Done
